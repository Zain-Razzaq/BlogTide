...
const BlogCard = ({ id, title, content, author, readTime }) => {
  ...
  return (
    <div className="text-customColor1 mx-10 my-20">
      <div className="h-[150px] flex items-center  relative">
        <span className="text-[14rem]  opacity-40 leading-[8rem] absolute top-0 -left-16">
          {title && title[0]}
        </span>
        <h2 className="text-5xl  font-lora font-semibold line-clamp-3">
          {title}
        </h2>
      </div>
